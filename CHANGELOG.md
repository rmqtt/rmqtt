...
- **Offline-Message Persistence Drops Are Now Observable**: `rmqtt-session-storage` hands every offline-message persist to a bounded executor (`OFFLINE_STORAGE_EXEC`: 8 workers, 10 000-slot queue) so a burst cannot grow the backlog without limit, and discards the task once that queue saturates — a deliberate trade-off against the OOM that unbounded `tokio::spawn` caused, but until now the only trace of it was a `WARN` throttled to every 1000th drop, invisible to any monitor. A new cumulative `Metrics` counter — `messages.offline.saves.dropped` in the API (Rust field `messages_offline_saves_dropped`, named after the `messages_*` family because its unit is one offline message's stored copy) — counts each discarded persist and travels the same paths as the other counters: `/api/v1/metrics` and `/api/v1/metrics/sum`, the Prometheus `rmqtt_metrics` family as `item="messages.offline.saves.dropped"` (per node and for the `all` aggregate), the metrics history, and the Dashboard's metric groups. Being cumulative, it is meant to be alerted on with `rate()` / `increase()`. What it reports is a **durability** downgrade rather than a delivery drop: the message is still delivered from the broker's memory, so only a further restart loses the copies that were never persisted. `rmqtt-test`'s stress suite now reads the counter after each run and reports it as a test note (never as a failure), and the standalone variant's broker starts `rmqtt-http-api` on its own port (6070, `configs/session-sled-stress`) so the single-broker case can report it too. Note for cluster operators: `Metrics` is exchanged between nodes as a typed message, so a mixed-version cluster cannot decode it — upgrade all nodes together.
