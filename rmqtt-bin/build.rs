...
fn plugins(decoded: &toml::Value) {
        ...
        .and_then(|plugins| plugins.as_table())
    {
        ...
        for (id, cfg) in plugins {
            ...
            inits.push(format!(
                "    {}::register(scx.clone(), r#\"{}\"#, {} || default_startups.contains(&String::from(r#\"{}\"#)), {}).await.map_err(|e| anyhow::anyhow!(format!(r#\"Failed to register '{}' plug-in, {{}} \"#, e.to_string())))?;",
    ...
    plugin_rs
        .write_all(b"pub(crate) async fn registers(scx: &rmqtt::context::ServerContext, default_startups: Vec<String>) -> rmqtt::Result<()>{\n")
        .unwrap();
