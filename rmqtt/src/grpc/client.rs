...
impl NodeGrpcClient {
    ...
    pub async fn new(
        ...
        log::debug!("rpc.client_timeout: {:?}", client_timeout);
        let concurrency_limit = client_concurrency_limit + 1;
        let endpoint = Channel::from_shared(format!("http://{}", server_addr))
            .map(|endpoint| endpoint.concurrency_limit(concurrency_limit).timeout(client_timeout))
    ...
    async fn _ping(&self) -> Result<()> {
        ...
        let _ =
            tokio::time::timeout(self.client_timeout, grpc_client.ping(tonic::Request::new(pb::Empty {})))
                .await
                .map_err(anyhow::Error::new)?
                .map_err(anyhow::Error::new)?;
        Ok(())
    ...
    #[inline]
    async fn _connect(endpoint: &Endpoint, client_timeout: Duration) -> Result<NodeServiceClientType> {
        let channel = tokio::time::timeout(client_timeout, endpoint.connect())
            .await
            .map_err(anyhow::Error::new)?
            .map_err(anyhow::Error::new)?;
    ...
    async fn connect(&self) -> Result<NodeServiceClientType> {
        ...
        let c = Self::_connect(&self.endpoint, self.client_timeout).await?;
    ...
    pub async fn batch_send_message(
        ...
        if !self.is_available() {
            return Err(anyhow!(format!("gRPC client connection unavailable {:?}", self.endpoint.uri())));
        }
    ...
    fn start(&self, mut rx: Receiver<(MessageType, Message, OneshotSender<Result<MessageReply>>)>) {
        ...
        let client_concurrency_limit = self.client_concurrency_limit;
        tokio::task::spawn(async move {
            ...
            while let Some((typ, msg, r_tx)) = rx.recv().await {
                ...
                if client.active_tasks() < client_concurrency_limit {
        ...
        if msgs.len() == 1 {
            ...
            match client.inner_batch_send_messages(msgs).await {
                Err(e) => {
                    for r_tx in r_txs {
                        if !r_tx.is_closed() {
                            if let Err(r) = r_tx.send(Err(anyhow!(e.to_string()))) {
